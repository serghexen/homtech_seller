  ...
  <Teleport to="body">
    <Transition name="topup-modal">
      <div v-if="open" class="topup-backdrop" @click.self="close">
        <section
          class="topup-modal"
          :class="{ 'topup-modal--payment': topup }"
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div v-else class="topup-payment">
            ...
            <p v-if="error" class="topup-error" role="alert">{{ error }}</p>
            <div v-else-if="isFinished" class="topup-result" :class="{ 'topup-result--success': topup.state === 'confirmed' }">
...
<style scoped>
.topup-backdrop { position: fixed; z-index: 140; inset: 0; display: grid; place-items: center; padding: 16px; overflow: auto; background: rgba(3,7,20,.78); backdrop-filter: blur(13px); }
.topup-modal { position: relative; width: min(100%,500px); max-height: calc(100dvh - 32px); padding: 24px; overflow-x: hidden; overflow-y: auto; border: 1px solid rgba(137,155,207,.34); border-radius: 26px; color: #eef3ff; background: radial-gradient(circle at 100% 0,rgba(59,91,198,.18),transparent 40%),linear-gradient(150deg,#111a38,#091027 74%); box-shadow: 0 35px 110px rgba(0,0,0,.55),inset 0 1px rgba(255,255,255,.04); }
.topup-modal--payment { width: min(100%,480px); padding: 22px 24px; }
.topup-close { position: absolute; z-index: 2; top: 15px; right: 15px; display: grid; width: 34px; height: 34px; place-items: center; padding: 0; border: 1px solid rgba(143,159,201,.25); border-radius: 11px; color: #aeb9d1; background: rgba(31,41,72,.72); font-size: 22px; line-height: 1; }
.topup-head { display: flex; align-items: center; gap: 12px; padding-right: 42px; }.topup-head__mark { display: grid; width: 48px; height: 48px; place-items: center; overflow: hidden; border: 1px solid rgba(130,146,193,.25); border-radius: 15px; background: #f5f1e8; }.topup-head__mark img { width: 32px; height: 26px; object-fit: contain; transform: scale(2); }.topup-head span { color: #6ee4c4; font-size: 9px; font-weight: 900; letter-spacing: .15em; }.topup-head h2 { margin: 2px 0 0; font-size: 26px; letter-spacing: -.035em; }
.topup-modal--payment .topup-head__mark { width: 44px; height: 44px; border-radius: 14px; }.topup-modal--payment .topup-head h2 { font-size: 24px; }
.topup-balance { display: grid; grid-template-columns: 1fr auto; align-items: end; gap: 2px 14px; margin: 18px 0; padding: 14px 16px; border: 1px solid rgba(123,145,208,.22); border-radius: 16px; background: rgba(21,31,65,.65); }.topup-balance span { color: #aab6d0; font-size: 11px; font-weight: 700; }.topup-balance strong { grid-row: 1 / 3; grid-column: 2; font-size: 20px; }.topup-balance small { color: #6f7e9f; font-size: 9px; }.topup-modal--payment .topup-balance { margin: 13px 0; padding: 11px 14px; }
...
.topup-payment { display: grid; gap: 11px; }.topup-status { display: flex; align-items: center; gap: 10px; padding: 9px 12px; border: 1px solid rgba(107,137,223,.25); border-radius: 13px; background: rgba(25,38,76,.58); }.topup-status__dot { width: 8px; height: 8px; border-radius: 50%; background: #f2bf5b; box-shadow: 0 0 0 4px rgba(242,191,91,.1); animation: topup-pulse 1.5s ease-in-out infinite; }.topup-status div { display: grid; gap: 1px; }.topup-status small { color: #7e8ba9; font-size: 7px; font-weight: 900; letter-spacing: .13em; }.topup-status strong { font-size: 12px; }.topup-status--confirmed .topup-status__dot { background: #55e2bd; box-shadow: 0 0 0 4px rgba(85,226,189,.11); animation: none; }
.topup-qr { display: grid; place-items: center; padding: 17px; border-radius: 18px; color: #1d1346; background: #f5f1e8; text-align: center; }.topup-qr__brand { width: 116px; height: auto; margin-bottom: 7px; object-fit: contain; }.topup-qr__image { width: min(100%,220px); aspect-ratio: 1; object-fit: contain; border: 8px solid #fff; border-radius: 11px; background: #fff; }.topup-qr strong { margin-top: 10px; font-size: 17px; }.topup-qr p { max-width: 330px; margin: 5px 0 0; color: #51496d; font-size: 10px; line-height: 1.4; }
.topup-result { display: grid; min-height: 170px; place-items: center; align-content: center; gap: 6px; padding: 22px; border: 1px solid rgba(242,174,88,.25); border-radius: 18px; background: rgba(87,54,30,.17); text-align: center; }.topup-result > span { display: grid; width: 44px; height: 44px; place-items: center; border-radius: 50%; color: #111a38; background: #eab563; font-size: 22px; font-weight: 900; }.topup-result strong { font-size: 17px; }.topup-result p { margin: 0; color: #aeb9d1; font-size: 11px; }.topup-result--success { border-color: rgba(85,226,189,.3); background: rgba(34,103,87,.18); }.topup-result--success > span { background: #55e2bd; }
...
.topup-again { justify-self: center; min-width: 150px; }.topup-footer { display: flex; justify-content: space-between; gap: 12px; margin-top: 16px; padding-top: 11px; border-top: 1px solid rgba(128,146,193,.16); color: #697794; font-size: 9px; }
...
@media (max-width:560px) { .topup-backdrop { align-items: end; padding: 0; }.topup-modal,.topup-modal--payment { width: 100%; max-height: 96dvh; padding: 18px 16px; overflow-y: auto; border-right: 0; border-bottom: 0; border-left: 0; border-radius: 23px 23px 0 0; }.topup-presets { grid-template-columns: repeat(2,1fr); }.topup-footer { flex-direction: column; gap: 3px; margin-top: 12px; padding-top: 9px; }.topup-qr { padding: 14px; }.topup-qr__brand { width: 100px; margin-bottom: 5px; }.topup-qr__image { width: min(100%,200px); }.topup-result { min-height: 150px; padding: 18px; } }
