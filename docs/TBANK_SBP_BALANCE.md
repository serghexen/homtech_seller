...
позицией «Аванс для оплаты услуг и цифровых товаров HomTech»: количество `1 шт.`,
сумма позиции равна сумме платежа, `PaymentMethod=advance`,
`PaymentObject=payment`, ставка берётся из `TBANK_RECEIPT_TAX`.
Согласованные значения окружения: `TBANK_RECEIPT_TAXATION=usn_income_outcome`
(УСН «Доходы минус расходы») и `TBANK_RECEIPT_TAX=vat105` (НДС 5/105).
НДС включён в сумму: Seller не увеличивает платёж и не уменьшает начисление
на баланс на сумму налога.

`TBANK_RECEIPT_EMAIL` — адрес получателя чека. Реальное значение хранится только
в серверном `.env`. Обновление `.env.example` не меняет действующее окружение.
При развёртывании перехода с услуги на аванс необходимо совместно обновить
код API и обе налоговые переменные в серверном `.env`, затем пересоздать API
с новым окружением. Уже созданные платежи и выпущенные чеки не переписываются.
Реквизиты итогового чека нужно сверить с настройками кассы «Чеков Т-Бизнеса».

Автоматическая отправка чека зачёта аванса при расходовании баланса в этом
изменении не реализована по запросу владельца. Это ограничение реализации,
а не подтверждение отсутствия обязанности сформировать такой чек: порядок
последующей фискализации должен быть согласован с бухгалтером.
