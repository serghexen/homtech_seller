...
5. Перепроверить все цены без покупки через Supplier Hub:

   ```bash
   python scripts/refresh_supplier_quotes.py --campaign-id 149196813
   ```

   После нулевого списка ошибок повторить с `--apply`. Endpoint вызывает только
...
несверенные политики либо quote вернул ошибку. Переключение webhook и включение
