...
Если CRM содержит настройки старых или архивных предложений, которых уже нет в
текущем каталоге Seller, строгий dry-run завершится ошибкой. После проверки списка
можно явно добавить `--skip-missing`: импортёр перенесёт только точно сопоставленные
карточки и выведет количество и первые идентификаторы пропущенных. Эти строки не
удаляются из CRM и могут быть доимпортированы после появления товара в каталоге Seller.
